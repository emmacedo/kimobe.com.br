...
type Props = {
    faturas: PaginatedData;
    resumo: Resumo;
    filtros: Filtros;
};

function enderecoResumo(c: FaturaData): string {
...
export default function FaturasIndex({ faturas, resumo, filtros }: Props) {
    ...
    function handleCancelar() {
        ...
        router.patch(`/financeiro/faturas/${cancelTarget.id}/cancelar`, {}, {
    ...
    return (
        <>
            ...
            <div className="space-y-4">
                <PageHeader titulo={isInquilino ? 'Minhas cobranças' : 'Cobranças'} subtitulo={faturas.total > 0 ? `${faturas.total} cobrança(s)` : undefined}>
                    {can.manage_faturas && (
                    ...
                    <div className="overflow-hidden rounded-[10px] border border-[#D8DCDA] bg-white">
                            ...
                            <TableBody>
                                {faturas.data.map((c) => (
                                    <TableRow
                                        ...
                                        onClick={() => router.visit(`/financeiro/faturas/${c.id}`)}
                                    >
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    <DropdownMenuItem asChild>
                                                        <Link href={`/financeiro/faturas/${c.id}`}>Ver detalhes</Link>
                                                    </DropdownMenuItem>
                                                    {can.manage_faturas && ['pendente', 'atrasado'].includes(c.status) && (
                                                        <>
                        ...
                        {faturas.last_page > 1 && (
                            <div className="flex items-center justify-between border-t border-[#EEF0EF] px-4 py-3">
                                <p className="text-xs text-[#8A918E]">Mostrando {faturas.from}–{faturas.to} de {faturas.total}</p>
                                <div className="flex items-center gap-1">
                                    {faturas.links.map((link, i) => {
                                        if (i === 0 || i === faturas.links.length - 1) {
                                            return (
