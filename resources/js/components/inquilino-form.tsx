...
export const dadosIniciaisInquilino: InquilinoFormData = {
    name: '',
    tipo_pessoa: 'pf',
    documento: '',
    telefone: '',
...
type Props = {
    dados: InquilinoFormData;
    errors: Record<string, string>;
    processing: boolean;
    onSubmit: (dados: InquilinoFormData) => void;
...
export function InquilinoForm({
    ...
    const [form, setForm] = useState<InquilinoFormData>(dados);
    ...
    function setField<K extends keyof InquilinoFormData>(key: K, value: InquilinoFormData[K]) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="rounded-[10px] border border-[#D8DCDA] bg-white p-5">
                <h2 className="mb-4 text-sm font-medium text-[#1E2D30]">Dados do inquilino</h2>
                    ...
                    <div>
                        ...
                        {avisoEmailPlaceholder && form.email === '' && (
                            <p className="mt-1 rounded-md bg-[#FFF4E5] p-2 text-xs text-[#8C5A10]">
                                Este inquilino foi cadastrado sem email. Adicione um email real se quiser que ele tenha acesso ao sistema futuramente.
                            </p>
                        )}
                    </div>
                </div>
